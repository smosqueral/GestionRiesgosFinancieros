...
```{r warning=FALSE}
estadisticas<-c("min","mean","max","var","sd","kurtosis","skewness")
n <- length(estadisticas)
m <- ncol(sm)
EstDesPrecios <- matrix(,nrow=n,ncol=m,dimnames = list(estadisticas,c("SP500","Apple","Microsoft","Amazon","AlphabetA","Tesla","AlphabetC","Meta","NVIDIA","BerkshireHathaway","UnitedHealth","Tbill")))
                                                
for (j in 1:n){
  for(i in 1:m){
      EstDesPrecios[j,i]=colStats(sm[,i], FUN = estadisticas[j])
  }
}
...
n <- length(estadisticas)
m <- ncol(ret)
EstDesRet <- matrix(,nrow=n,ncol=m,dimnames = list(estadisticas,c("SP500","Apple","Microsoft","Amazon","AlphabetA","Tesla","AlphabetC","Meta","NVIDIA","BerkshireHathaway","UnitedHealth","Tbill")))
                                                
for (j in 1:n){
  for(i in 1:m){
      EstDesRet[j,i]=colStats(ret[,i], FUN = estadisticas[j])
  }
}
...
```{r}
rf <- sm$IRX.Adjusted
rf <-(1+rf/2)^(1/180)-1  #La tasa se encuentra en NASV y se pasa a diaria
rf <- log(1+rf)*100       #La tasa diaria se convierte a continua 

rm <- ret$GSPC.Adjusted
...
colnames(EstDesRetornos) <- c("Retornos SP500")
...
```
```{r}
modelo.precios = arima(sp, order=c(2,1,4))
...
```{r}
modelo.retornos = arima(ret, order=c(2,0,4))
...
```{r}
forecast<- forecast(modelo.precios, h=50)
plot(forecast)
...
```{r}
forecastret<- forecast(modelo.retornos, h=50)
plot(forecastret)
...
sp.volh2 <- as.matrix(sp.volh[2:t,1])
sp.volRM2 <- sqrt(sp.volRM[2:t,1])
